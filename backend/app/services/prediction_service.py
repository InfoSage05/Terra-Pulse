...
async def predict_price(db: AsyncSession, input_data: PricePredictionInput) -> Optional[PricePredictionOutput]:
    ...
    row = (await db.execute(query, {"area_id": input_data.area_id})).first()
    if not row:
