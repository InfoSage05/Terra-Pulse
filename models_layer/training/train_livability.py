...
def compute_livability_model():
    print("Computing livability model (blend)...")
    df = get_area_features()
    
    valid_df = df[df['livability_signal'].notnull()].copy()
    if valid_df.empty:
        print("No valid agent data for livability model.")
        return
    ...
    metadata = ModelMetadata(
        model_type=ModelType.LIVABILITY_SCORE,
        version=version,
        trained_at=datetime.now(),
        training_row_count=len(valid_df),
        feature_names=['livability_signal', 'avg_price', 'total_crime'],
